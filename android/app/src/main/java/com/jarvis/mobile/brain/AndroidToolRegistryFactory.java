...
public final class AndroidToolRegistryFactory {
    ...
    public static ToolRegistry create(Context context, ExternalResearchGateway research, ConversationalCallTransport callTransport) {
        ...
        register(registry, "ui_click", true, Set.of("click visible control", "tap visible control"), Set.of("target"), "After approval, click exactly one uniquely matching visible Android control", ToolExecutionClass.CONSEQUENTIAL, args -> accessibility.click(args.get("target")));
        register(registry, "ui_type", true, Set.of("type into field", "enter into field"), Set.of("text"), "After approval, type into the one uniquely selected editable Android field", ToolExecutionClass.CONSEQUENTIAL, args -> accessibility.type(args.get("text")));
    ...
    private static boolean isFailureOutcome(String lower) { return lower.contains("couldn’t") || lower.contains("couldn't") || lower.contains("could not") || lower.contains("failed") || lower.contains("blocked") || lower.startsWith("no compatible") || lower.startsWith("tell me") || lower.startsWith("enable ") || lower.contains("unavailable") || lower.contains("must be") || lower.contains("too large") || lower.contains("does not expose") || lower.startsWith("unsupported "); }
}
