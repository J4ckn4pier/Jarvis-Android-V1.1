...
for attempt in $(seq 1 30); do
  ...
  if grep -q 'JARVIS_COMMAND_RESULT.*You can speak naturally' "$OUTPUT/emulator-command-logcat.txt" && \
     grep -q 'JARVIS_COMMAND_RESULT.*call contacts' "$OUTPUT/emulator-command-logcat.txt"; then COMMAND_PASSED=1; break; fi
  sleep 1
done
...
# Shared-brain migration gate: production MainActivity/JarvisBrain must be backed by
# brain-core, not only the legacy LocalIntentEngine switch. The marker is emitted by
# the shared runtime path and the basic conversational test must avoid old failure copy.
adb shell am force-stop com.itsmylab.jarvis
adb logcat -c
adb shell am start -W \
  -n com.itsmylab.jarvis/com.jarvis.mobile.MainActivity \
  --es jarvis_test_command "'how are you'" \
  | tee "$OUTPUT/emulator-shared-brain-launch.txt"
...
for attempt in $(seq 1 30); do
  ...
  if grep -q 'JARVIS_SHARED_BRAIN_ACTIVE' "$OUTPUT/emulator-shared-brain-logcat.txt" && \
     grep -q 'JARVIS_COMMAND_RESULT' "$OUTPUT/emulator-shared-brain-logcat.txt" && \
     ! grep -Eiq 'JARVIS_COMMAND_RESULT.*(reliable interpretation|no framework)' "$OUTPUT/emulator-shared-brain-logcat.txt"; then
...
adb logcat -c
adb shell am start -W -n com.itsmylab.jarvis/com.jarvis.mobile.MainActivity | tee "$OUTPUT/emulator-normal-launch.txt"
...
adb shell cmd role get-role-holders android.app.role.ASSISTANT | tee "$OUTPUT/emulator-assistant-role.txt" | grep -q '^com.itsmylab.jarvis$'
sleep 3
...
adb shell dumpsys package com.itsmylab.jarvis | tee "$OUTPUT/emulator-package-dumpsys.txt" || true
