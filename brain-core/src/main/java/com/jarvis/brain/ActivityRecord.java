...
public record ActivityRecord(String id,Instant at,String title,Status status,String detail,Map<String,String> evidence){
 public enum Status{DONE,NEEDS_APPROVAL,NEEDS_DECISION,FAILED,IGNORED}
 ...
 public static Status from(BrainRuntime.Status status){return switch(status){case COMPLETED->Status.DONE;case APPROVAL_REQUIRED->Status.NEEDS_APPROVAL;case RECOVERY_REQUIRED->Status.NEEDS_DECISION;case FAILED->Status.FAILED;case IGNORED->Status.IGNORED;};}
}
