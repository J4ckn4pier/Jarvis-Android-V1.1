...
public final class RuntimeApprovalConversation{
 ...
 private RuntimeSurfacePresentation projectPendingDecisionIfNeeded(BrainRuntime.Result result){RuntimeSurfacePresentation surface=RuntimeSurfacePresentation.from(result);if(result.status()!=BrainRuntime.Status.COMPLETED)return surface;if(runtime.hasPendingApproval())return new RuntimeSurfacePresentation(AssistantSurfaceState.AWAITING_APPROVAL,result.text(),"Pending action still requires explicit approval",RuntimeSurfaceAction.APPROVE,RuntimeSurfaceAction.CANCEL);if(runtime.hasPendingRecovery())return new RuntimeSurfacePresentation(AssistantSurfaceState.NEEDS_INPUT,result.text(),"Pending action still requires an explicit recovery decision",RuntimeSurfaceAction.RETRY,RuntimeSurfaceAction.CANCEL);return surface;}
