...
public final class LongTermMemoryStore {
    ...
    public synchronized void put(RichMemory memory) {
        ...
        if (memory.type() != MemoryType.EPISODE) for (int i=0;i<list.size();i++) { RichMemory existing=list.get(i); if (existing.validUntil()==null && existing.validFrom().isBefore(memory.validFrom()) && !existing.content().equals(memory.content())) list.set(i, existing.closeAt(memory.validFrom())); }
        list.add(memory); list.sort(Comparator.comparing(RichMemory::validFrom));
    }
    ...
    public synchronized boolean archive(String key, Instant when) {
        List<RichMemory> list=byKey.get(key); if(list==null||list.isEmpty()) return false; Instant at=when==null?Instant.now():when; boolean changed=false;
        for(int i=0;i<list.size();i++){RichMemory m=list.get(i);if(m.validUntil()==null){list.set(i,m.closeAt(at));changed=true;}}
        return changed;
    }
    public synchronized void observeRoutine(String key,String content,Set<String> tags,Instant observedAt) { List<RichMemory> list=byKey.computeIfAbsent(key,k->new ArrayList<>()); for(int i=0;i<list.size();i++){RichMemory existing=list.get(i);if(existing.type()==MemoryType.ROUTINE&&existing.validUntil()==null&&existing.content().equals(content)){int count=existing.evidenceCount()+1;double confidence=Math.min(0.98,0.45+0.09*count);list.set(i,existing.withEvidence(count,confidence));return;}} list.add(new RichMemory(key,MemoryType.ROUTINE,content,"observed",0.54,0.72,observedAt,null,tags,1)); }
    ...
    public synchronized int prune(MemoryRetentionPolicy policy, Instant now) { if (policy==null||now==null) return 0; int archived=0; for(List<RichMemory> list:byKey.values()) for(int i=0;i<list.size();i++) { RichMemory memory=list.get(i); if(memory.validUntil()==null && policy.shouldPrune(memory,now)) { list.set(i,memory.closeAt(now)); archived++; } } return archived; }
    ...
    public synchronized List<RichMemory> retrieve(String query,Instant when,int limit){Set<String> q=terms(query);List<Scored> scored=new ArrayList<>();for(Map.Entry<String,List<RichMemory>> entry:byKey.entrySet()){List<RichMemory> list=entry.getValue();int currentIndex=currentIndex(list,when);if(currentIndex<0)continue;RichMemory m=list.get(currentIndex);double semantic=overlap(q,memoryTerms(m));if(semantic<=0)continue;RichMemory touched=m.touch(when);if(touched!=m){list.set(currentIndex,touched);m=touched;}double sourceTrust=(m.source().equals("user-stated")||m.source().equals("manual-user-entry"))?1.0:m.type()==MemoryType.INFERENCE?m.confidence():0.82;double stability=switch(m.type()){case PREFERENCE,FACT,RELATIONSHIP,PROCEDURE,GOAL->1.0;case ROUTINE->Math.min(1.0,0.55+0.08*m.evidenceCount());case EPISODE->0.55;case INFERENCE->0.50;};long ageDays=Math.max(0,Duration.between(m.validFrom(),when).toDays());double recency=1.0/(1.0+ageDays/120.0);double score=0.42*semantic+0.24*m.importance()+0.15*m.confidence()+0.10*sourceTrust+0.06*stability+0.03*recency;scored.add(new Scored(m,score));}scored.sort(Comparator.comparingDouble(Scored::score).reversed());return scored.stream().limit(Math.max(0,limit)).map(Scored::memory).toList();}
