...
/**
 * Cheap semantic-reflex layer for obvious local capabilities. It intentionally handles only
 * high-confidence paraphrases; ambiguous goals fall through to the reasoning provider.
 */
public final class SemanticGoalInterpreter {
    public Optional<Plan> interpret(String utterance) {
        ...
        if (isDialer(lower)) {
            return Optional.of(new Plan("Open the phone dialer", List.of(new PlanStep("open_dialer"))));
        }

        String calendarWhen = calendarWhen(lower);
        if (calendarWhen != null) {
            return Optional.of(new Plan("Read calendar agenda",
                    List.of(new PlanStep("calendar_query", Map.of("when", calendarWhen), false))));
        }

        String destination = navigationDestination(raw, lower);
        if (!destination.isBlank()) {
            return Optional.of(new Plan("Navigate to destination",
        ...
        if (lower.contains("torch") || lower.contains("flashlight")) {
            ...
            if (!state.isBlank()) {
                return Optional.of(new Plan("Set flashlight",
        ...
        if (isFoodDiscovery(lower)) {
            ...
            return Optional.of(new Plan("Find a good nearby place to eat",
                    List.of(
                            new PlanStep("discover_places", args, false),
    ...
    private static boolean isDialer(String lower) {
        ...
        return lower.contains("open")
                || lower.contains("use to make")
                || lower.contains("make a phone call")
    ...
    private static String navigationDestination(String raw, String lower) {
        ...
        for (String cue : cues) {
            ...
            if (idx >= 0) return raw.substring(Math.min(raw.length(), idx + cue.length())).trim();
        }
        return "";
    }
    ...
    private static boolean onLanguage(String lower) {
        return lower.contains("turn on") || lower.contains("switch on") || lower.contains("enable");
    }
