...
public final class PopupOverlayController{
 ...
 public synchronized void show(String title,String detail){if(title==null||title.isBlank())throw new IllegalArgumentException("title required");state=new PopupOverlayState(true,title,detail);}
 public synchronized void dismiss(){state=PopupOverlayState.hidden();}
 public synchronized PopupOverlayOutcome onAction(PopupOverlayAction action){if(action==null)throw new IllegalArgumentException("action required");if(!state.visible())throw new IllegalStateException("overlay is not visible");return switch(action){case DETAILS->PopupOverlayOutcome.SHOW_DETAILS;case LATER->{dismiss();yield PopupOverlayOutcome.DEFERRED;}case YES->{dismiss();yield PopupOverlayOutcome.APPROVAL_REQUESTED;}};}
}
