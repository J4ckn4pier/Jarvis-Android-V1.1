...
public final class ResumableExecutionTest {
    ...
    private static void blankRetryableConsequentialFailureStillReportsRetry() {
        ToolRegistry registry = new ToolRegistry();
        registry.register(new ToolSpec("send_message", true, Set.of(), Set.of(), "send"),
                (a,c) -> ToolResult.retryableFailure("   "));
        ApprovalGate gate = new ApprovalGate();
        ResumablePlanExecutor executor = new ResumablePlanExecutor(registry, gate);
        ExecutionCursor cursor = executor.start(new Plan("send", List.of(new PlanStep("send_message", Map.of(), true))));
        check(executor.run(cursor, new ExecutionContext()).status() == ExecutionReport.Status.APPROVAL_REQUIRED,
                "consequential action first waits for approval");
        gate.approve("send_message");
        ExecutionReport retry = executor.run(cursor, new ExecutionContext());
        check(retry.status() == ExecutionReport.Status.APPROVAL_REQUIRED,
                "blank-detail temporary failure still requires fresh approval");
        check(!retry.failureDetail().isBlank(),
                "retry report must remain distinguishable from an initial approval even when provider omits failure detail");
    }
