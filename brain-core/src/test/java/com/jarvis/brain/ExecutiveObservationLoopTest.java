...
public final class ExecutiveObservationLoopTest {
    ...
    private static void successfulSafeToolWithBlankOutputBecomesFailureObservation() {
        ToolRegistry registry = new ToolRegistry();
        registry.register(new ToolSpec("lookup", false, Set.of(), Set.of(), "lookup"),
                (args, ctx) -> ToolResult.success("   "));
        final int[] reasons = {0};
        final String[] recoveryContext = {""};
        ReasoningRouter router = request -> {
            reasons[0]++;
        ...
        ExecutiveOutcome outcome = new ExecutiveObservationLoop(router, registry, new ApprovalGate(), 3).run("verify it", "");
        check(outcome.status() == ExecutiveOutcome.Status.ANSWERED,
                "blank success output should stay recoverable through a subsequent reasoning turn");
        check(recoveryContext[0].contains("FAILED"),
                "whitespace-only tool output must not be accepted as successful evidence");
        check(recoveryContext[0].toLowerCase().contains("output"),
                "blank success output should be explained as missing usable output");
    }
