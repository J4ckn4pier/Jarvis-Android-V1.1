...
public final class ProviderAttemptBudgetTest {
    ...
    private static void malformedPreferredProviderFallsBackWithinExplicitBudget() {
        ...
        PolicyProviderRouter router = new PolicyProviderRouter(
                List.of(new ProviderRoute(local, ProviderTier.LOCAL, 0), new ProviderRoute(fallback, ProviderTier.FREE_EXTERNAL, 0)),
    ...
    private static void budgetPreventsWalkingEntireProviderFleet() {
        ...
        PolicyProviderRouter router = new PolicyProviderRouter(
                List.of(new ProviderRoute(p1, ProviderTier.LOCAL, 0), new ProviderRoute(p2, ProviderTier.FREE_EXTERNAL, 0), new ProviderRoute(p3, ProviderTier.FREE_EXTERNAL, 1)),
    ...
    private static ReasoningProvider provider(String id, Call call) {
        return new ReasoningProvider() {
            @Override public String id() { return id; }
            @Override public boolean available() { return true; }
            @Override public ReasoningResult reason(ReasoningRequest request) { return call.run(); }
        };
    }
