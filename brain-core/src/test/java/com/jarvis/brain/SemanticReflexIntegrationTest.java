...
public final class SemanticReflexIntegrationTest {
    ...
    private static AssistantCore core(int[] providerCalls) {
        ...
        ReasoningRouter provider = request -> {
            providerCalls[0]++;
            return new ReasoningResult("local", "I can reason about that.", null);
        };
    ...
    private static void paraphrasedDialerRequestAvoidsLiteralAppNameFailure() {
        int[] calls = {0};
        BrainResponse r = core(calls).handle("open whatever I use to make a phone call");
    ...
    private static void naturalCalendarQuestionRoutesOffline() {
        int[] calls = {0};
        BrainResponse r = core(calls).handle("what does my day look like tomorrow");
        check(r.kind() == BrainResponse.Kind.ACTION_PLAN && r.plan().steps().get(0).tool().equals("calendar_query"),
                "natural agenda question should route calendar query");
    ...
    private static void naturalNavigationRequestRoutesOffline() {
        int[] calls = {0};
        BrainResponse r = core(calls).handle("get me to the nearest gas station");
        check(r.kind() == BrainResponse.Kind.ACTION_PLAN && r.plan().steps().get(0).tool().equals("navigate"),
                "natural get-me-to phrasing should route navigation");
        check(r.plan().steps().get(0).arguments().get("destination").toLowerCase().contains("gas station"),
                "navigation should retain destination semantics");
    ...
    private static void torchParaphraseRoutesOffline() {
        int[] calls = {0};
        BrainResponse r = core(calls).handle("kill the torch");
        check(r.kind() == BrainResponse.Kind.ACTION_PLAN && r.plan().steps().get(0).tool().equals("set_flashlight"),
                "torch paraphrase should resolve flashlight capability");
    ...
    private static void ambiguousRequestStillFallsThroughToReasoning() {
        int[] calls = {0};
        BrainResponse r = core(calls).handle("figure out the best way to reorganize my week");
